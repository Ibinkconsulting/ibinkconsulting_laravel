            ...
            <div class="card-body">
                <div class="table-responsive p-4">
                    ...
                    <div class="d-flex justify-content-end">
                        <a href="{{ route('admin.property.create') }}" class="btn btn-primary" type="button">
...
@push('script')
    ...
    <script>
        ...
        function showStatusChangeAlert(event, id) {
            ...
            let statusUrl = '{{ route('admin.property.status', ':id') }}'.replace(':id', id);

            Swal.fire({
                title: 'Are you sure?',
                text: "You want to change the status of Property.",
                icon: 'warning',
        ...
        function deleteRecord(id) {
            ...
            let deleteUrl = '{{ route('admin.property.destroy', ':id') }}'.replace(':id', id);
    ...
    {{-- Change Status --}}
    <script>
        function changeAvailability(element, id) {
            let availlability = $(element).val();

            // Ajax Call
            let route = "{{ route('admin.property.change.availlability', ':id') }}"
            $.ajax({
                type: 'POST',
                url: route.replace(':id', id),
                data: {
                    _token: '{{ csrf_token() }}',
                    "availibility": availlability
                },
                success: function(response) {
                    if (response.success) {
